...
/// Whether a script hands `otp repack` an installed OTP root to repack.
///
/// `--root` selects `catalog::repack_from_root`, which reads a runtime that is
/// already on the machine and never asks `upstream_asset` to name a download —
/// so the rule below has nothing to say about it. F1 added that path precisely
/// so that the macOS and Windows release jobs could repack the runtime
/// `erlef/setup-beam` installed for them, which is why they ask for targets no
/// upstream asset covers and are right to.
///
/// Recognised as a word, not a substring: `--rootless` is not `--root`.
fn resolves_a_local_root(script: &str) -> bool {
    shell_code(script)
        .split_whitespace()
        .any(|word| word == "--root")
}
...
#[test]
fn a_local_root_is_not_an_upstream_download() {
    assert!(resolves_a_local_root(
        "cargo run -- otp repack --targets macos-aarch64 --root \"$INSTALL_DIR_FOR_OTP\" --out d\n"
    ));
    assert!(!resolves_a_local_root(
        "cargo run -- otp repack --upstream-tag OTP-29.0.5 --targets linux-x86_64-gnu --out d\n"
    ));
    assert!(
        !resolves_a_local_root("cargo run -- otp repack --rootless --targets linux-x86_64-gnu\n"),
        "a longer flag that begins the same way is a different flag"
    );
}
    ...
    for relative in yaml_files_under(".github/workflows") {
        ...
        for (id, job) in jobs {
            ...
            for step in steps {
                ...
                if resolves_a_local_root(script) {
                    continue;
                }
