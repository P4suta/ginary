...
/// The committed shell scripts a step's own script invokes.
///
/// A word ending in `.sh` that names a path under `scripts/` and exists in the
/// tree. Nothing else: a rule that followed every word would read whatever a
/// runner happens to have, and the subject here is what *this repository* says
/// the job does.
fn invoked_scripts(run: &str) -> Vec<String> {
    let mut found = Vec::new();
    for word in run.split_whitespace() {
        let word = word.trim_matches(['"', '\'', '`', '(', ')', ';', '&']);
        if !word.ends_with(".sh") || !word.starts_with("scripts/") {
            continue;
        }
        if crate::common::repo::root().join(word).is_file() && !found.contains(&word.to_owned()) {
...
/// One step, with the body of every committed script it runs appended.
///
/// F1 moved the macOS job's body out of an inline `run:` and into
/// `scripts/ci/macos-smoke.sh`, to keep actionlint off a long stdin pipe. Every
/// rule in this file reads a step's script, so that move made the `macos` job
/// look like one that packages nothing and starts nothing — and this file's
/// rule then demanded that `README.md` go back to saying macOS "has never been
/// run on a Mac", which by then was false. A guard that inverts when a script
/// is factored out is a guard that is not watching, so it follows the script.
fn with_scripts(step: &WorkflowStep) -> WorkflowStep {
    let mut extended = step.clone();
    for path in invoked_scripts(&step.run) {
        extended.run.push('\n');
        extended.run.push_str(&read(&path));
    }
    extended
}
...
fn job_launches_an_artifact(job: &str) -> bool {
    let steps: Vec<WorkflowStep> = steps_of(job).iter().map(with_scripts).collect();
    assert!(
        !steps.is_empty(),
...
/// The document that *affirms* a platform is proved, as opposed to the one
/// that must not deny it.
///
/// Both documents are subject to the rule above, which forbids a stale denial.
/// This one is not: F1 rewrote `README.md` around a matrix of the execution
/// evidence each target *requires*, under a sentence saying that "workflow
/// definitions describe the required checks; they do not establish that a
/// particular commit has passed them" — a claim `tests/v1_readiness.rs` pins.
/// A README written that way makes no assertion that a launch happened, so
/// asking it to name the image one happened on is asking it to re-make the
/// claim it deliberately stopped making. The fail-closed checklist is where
/// that claim lives, and it is the one held to naming its evidence.
const AFFIRMING_DOCUMENT: &str = "docs/dev/v1-readiness.md";
...
fn each_launched_platform_names_the_runner_that_launched_it() {
    ...
    for (job, runners) in [
        ...
        let text = read(AFFIRMING_DOCUMENT);
        for runner in runners {
            assert!(
                text.contains(runner),
                "{AFFIRMING_DOCUMENT} says the `{job}` launch is proved and never names \
                 `{runner}`, the image it is proved on"
            );
        }
    }
}
