...
| `GINARY_OFFLINE=1` | implemented | Forbids every fetch. `download::Net` refuses before a socket is opened and the error names the URL and the file it was for, so a build that would have gone to the network says what it wanted rather than reaching for it. Command-line `--offline` can only turn it on: a build asked to stay offline is not put back on the network by an environment. |
